...
ProcFireball_Bubble:
      lda PlayerStatus           ;check player's status [AD5607]
      cmp #$02                   ; [C902]
      bcc ProcAirBubbles         ;if not fiery, branch [9043]
      lda A_B_Buttons            ; [A50A]
      and #B_Button              ;check for b button pressed [2940]
      beq ProcFireballs          ;branch if not pressed [F033]
      and PreviousA_B_Buttons    ; [250D]
      bne ProcFireballs          ;if button pressed in previous frame, branch [D02F]
      lda FireballCounter        ;load fireball counter [ADCE06]
      and #%00000001             ;get LSB and use as offset for buffer [2901]
      tax                        ; [AA]
      lda Fireball_State,x       ;load fireball state [B524]
      bne ProcFireballs          ;if not inactive, branch [D025]
      ldy Player_Y_HighPos       ;if player too high or too low, branch [A4B5]
      dey                        ; [88]
      bne ProcFireballs          ; [D020]
      lda CrouchingFlag          ;if player crouching, branch [AD1407]
      bne ProcFireballs          ; [D01B]
      lda Player_State           ;if player's state = climbing, branch [A51D]
      cmp #$03                   ; [C903]
      beq ProcFireballs          ; [F015]
...
InitFireballExplode:
      lda #$80                    ; [A980]
      sta Fireball_State,x        ;set exploding flag in fireball's state [9524]
      lda #Sfx_Bump               ; [A902]
      sta Square1SoundQueue       ;load bump sound [85FF]
      rts                         ;leave [60]
