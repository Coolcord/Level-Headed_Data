...
int main(int argc, char *argv[]) {
    ...
    for (QString file : files) {
        ...
        if (!graphicsPatchesLocation.isEmpty()) {
            if (file == "Original (by Nintendo).hexp" && !ripper.Dump_Tile_Order_Map(graphicsPatchesLocation+"/../../Level-Headed/SMB1/SMB1_Writer/Graphics_Offsets.cpp")) { qWarning() << "Failed to write used offsets"; return 1; }
        }
    }
